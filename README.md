...
| 📁 **[Bài Luyện Tập 6](./Bài%20Luyện%20Tập%206)** | Components, Props và State trong React Native | - Khái niệm Component, tính độc lập, đóng gói, tái sử dụng.<br>- So sánh Functional Component vs Class Component; Vai trò `useState` và `useEffect`.<br>- **Bài 1:** Component `Greeting` truyền dữ liệu qua Props.<br>- **Bài 2:** Component `StudentInfo` tái sử dụng hiển thị danh sách sinh viên.<br>- **Bài 3:** Component `CounterHook` sử dụng `useState` kích hoạt Re-render.<br>- File `App.js` tích hợp toàn diện & Mockup giao diện điện thoại. |
| 📁 **[Bài Luyện Tập 7](./Bài%20Luyện%20Tập%207)** | Vòng đời Component, Lồng Component & Form Validation | - Vòng đời Component: Mounting, Updating, Unmounting (`useEffect`).<br>- Khái niệm Controlled Component & Lợi ích xử lý dữ liệu nhập.<br>- **Bài 1:** Controlled Component nhập họ tên với `TextInput` & `useState`.<br>- **Bài 2:** Lồng Component (`Avatar` trong `UserProfile`).<br>- **Bài 3:** Form Đăng ký tài khoản với đầy đủ Validation & **2 ảnh nộp (Ảnh Lỗi & Ảnh Đúng)**. |
